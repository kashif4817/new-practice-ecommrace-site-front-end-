...
import { Crown, ArrowLeft, Sparkles, Eye, EyeOff, Lock, CheckCircle2 } from "lucide-react";
...
export default function ResetPasswordPage({ onSuccess, onBack }) {
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-stone-50 px-6">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-2xl font-bold text-stone-800 mb-3">Password Reset!</h2>
      ...
      <div className="hidden lg:flex w-[420px] shrink-0 bg-gradient-to-b from-stone-900 via-stone-900 to-stone-950 flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-10">
          ...
          <p className="text-stone-400 text-sm leading-relaxed">
            Create a strong, unique password to keep your LUXE admin account secure.
          </p>

          {/* Password tips */}
          <div className="mt-10 bg-white/5 border border-white/8 rounded-2xl p-5 space-y-3">
            <p className="text-[10px] uppercase tracking-widest text-stone-500 font-semibold mb-4">Password Tips</p>
            {rules.map((rule) => (
              <div key={rule.label} className="flex items-center gap-2.5">
                <div className={`w-4 h-4 rounded-full flex items-center justify-center transition-all ${
                  ...
                  {rule.test(password) && <span className="text-emerald-400 text-[8px]">✓</span>}
                </div>
                <span className={`text-xs transition-colors ${rule.test(password) ? "text-emerald-400" : "text-stone-600"}`}>
                  {rule.label}
                </span>
              </div>
            ))}
          ...
          <div className="mt-8 space-y-4">
            {[
              ...
              <div key={s.step} className="flex items-center gap-3">
                ...
                {s.active && <div className="flex-1 h-px bg-gradient-to-r from-amber-500/40 to-transparent" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs tracking-wider uppercase text-amber-600 font-medium">Final Step</span>
            </div>
            <h2 className="text-3xl font-bold text-stone-800 tracking-tight">Reset password</h2>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className={`relative flex items-center rounded-xl border-2 transition-all duration-200 bg-white ${
                ...
                <Lock className={`absolute left-4 w-4 h-4 transition-colors ${focusedField === "password" ? "text-amber-500" : "text-stone-400"}`} />
                <input
            ...
            <div>
              ...
              <div className={`relative flex items-center rounded-xl border-2 transition-all duration-200 bg-white ${
                ...
                <Lock className={`absolute left-4 w-4 h-4 transition-colors ${focusedField === "confirm" ? "text-amber-500" : "text-stone-400"}`} />
                <input
              ...
              {confirm.length > 0 && (
                <p className={`text-xs mt-2 ${passwordsMatch ? "text-emerald-600" : "text-red-500"}`}>
