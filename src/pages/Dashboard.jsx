...
import {
  DollarSign, ShoppingCart, UserCheck, Target,
  ArrowUpRight, ArrowDownRight, TrendingUp,
  MoreHorizontal, ChevronRight, Sparkles, Filter, Package,
  CheckCircle2, Truck, RefreshCw, Clock, XCircle,
  Star, Heart, AlertCircle,
} from "lucide-react";

// ─── Data ────────────────────────────────────────────────────────────────────

const statsData = [
  { label: "Total Revenue",    value: "$48,265", change: "+12.5%", trend: "up",   icon: DollarSign,  bgLight: "bg-amber-50",   textColor: "text-amber-600"  },
  { label: "Total Orders",     value: "1,847",   change: "+8.2%",  trend: "up",   icon: ShoppingCart,bgLight: "bg-emerald-50", textColor: "text-emerald-600"},
  { label: "New Customers",    value: "462",     change: "-2.4%",  trend: "down", icon: UserCheck,   bgLight: "bg-blue-50",    textColor: "text-blue-600"   },
  { label: "Conversion Rate",  value: "3.24%",   change: "+0.8%",  trend: "up",   icon: Target,      bgLight: "bg-purple-50",  textColor: "text-purple-600" },
];

const revenueData = [
  { month: "Jan", value: 65 }, { month: "Feb", value: 45 }, { month: "Mar", value: 78 },
  { month: "Apr", value: 52 }, { month: "May", value: 88 }, { month: "Jun", value: 72 },
  { month: "Jul", value: 95 }, { month: "Aug", value: 68 }, { month: "Sep", value: 82 },
  { month: "Oct", value: 90 }, { month: "Nov", value: 76 }, { month: "Dec", value: 100 },
];
...
const quickStats = [
  { label: "Pending Orders",    value: "24", icon: Clock,        color: "text-amber-600",  bg: "bg-amber-50"  },
  { label: "Out for Delivery",  value: "18", icon: Truck,        color: "text-blue-600",   bg: "bg-blue-50"   },
  { label: "Completed Today",   value: "47", icon: CheckCircle2, color: "text-emerald-600",bg: "bg-emerald-50"},
  { label: "Returns",           value: "3",  icon: RefreshCw,    color: "text-red-500",    bg: "bg-red-50"    },
];
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs tracking-wider uppercase text-amber-600 font-medium">Welcome back</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-6">
        {statsData.map((stat) => (
          <div key={stat.label} className="bg-white rounded-2xl border border-stone-200/60 p-5 lg:p-6 hover:shadow-lg hover:shadow-stone-200/50 transition-all duration-300">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 lg:gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-stone-200/60 p-5 lg:p-6">
              ...
              {["weekly", "monthly", "yearly"].map((p) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${
                    chartPeriod === p ? "bg-white text-stone-800 shadow-sm" : "text-stone-500 hover:text-stone-700"
                  }`}
                >
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-stone-100">
            <div><p className="text-xs text-stone-500">This Month</p><p className="text-lg font-bold text-stone-800">$48,265</p></div>
            <div><p className="text-xs text-stone-500">Last Month</p><p className="text-lg font-bold text-stone-800">$42,890</p></div>
            <div>
              <p className="text-xs text-stone-500">Growth</p>
              <p className="text-lg font-bold text-emerald-600 flex items-center gap-1"><TrendingUp className="w-4 h-4" />12.5%</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-200/60 p-5 lg:p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-stone-400 hover:text-stone-600 p-1 cursor-pointer"><MoreHorizontal className="w-5 h-5" /></button>
          </div>
              ...
              <thead>
                <tr className="border-b border-stone-100">
                  {["Order", "Customer", "Product", "Amount", "Status"].map((h, i) => (
                    <th key={h} className={`text-left text-[11px] uppercase tracking-wider text-stone-400 font-semibold px-6 py-3 ${i === 2 ? "hidden md:table-cell" : ""}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
        ...
        <div className="bg-white rounded-2xl border border-stone-200/60 p-5 lg:p-6">
          ...
          <div className="space-y-1">
            {activityFeed.map((a, i) => (
              <div key={i} className="flex gap-3 p-2.5 rounded-xl hover:bg-stone-50 transition-colors">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                  a.type === "order"    ? "bg-amber-50 text-amber-600"   :
                  a.type === "review"  ? "bg-yellow-50 text-yellow-600" :
                  a.type === "customer"? "bg-emerald-50 text-emerald-600":
                  a.type === "stock"   ? "bg-red-50 text-red-500"       :
                  a.type === "wishlist"? "bg-pink-50 text-pink-500"     :
                                         "bg-blue-50 text-blue-600"
                }`}>
                  <a.icon className="w-4 h-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-stone-700 leading-snug">{a.message}</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        {quickStats.map((item) => (
          <div key={item.label} className="bg-white rounded-2xl border border-stone-200/60 p-4 lg:p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
